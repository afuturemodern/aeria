...
raw_name = input("Enter a soundcloud profile to analyze: ")
    ...
    for profile in profiles_to_enqueue:
        ...
        if username:
            ...
            print(
                ...
                + ", ".join(
                    [
                        scac.id2username(user)
                        if isinstance(scac.id2username(user), str)
                        else str(user)
                        for user in newFollowings
                    ]
                )
            )
            ...
            print(
                ...
                + ", ".join(
                    [
                        scac.id2username(comment, "comments")
                        if isinstance(scac.id2username(comment, "comments"), str)
                        else str(comment)
            ...
            print(
                ...
                + ", ".join(
                    [
                        scac.id2username(track, "tracks")
                        if isinstance(scac.id2username(track, "tracks"), str)
                        else str(track)
...
print(
    "The profile graph currently contains "
    + str(len(profileGraph.nodes()))
    + " profiles."
)
...
print(
    "The profile graph currently contains "
    + str(nx.number_strongly_connected_components(profileGraph))
