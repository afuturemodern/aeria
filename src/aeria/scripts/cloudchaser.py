...
from resources.cc_mp_classes import Consumer, bookTasks
...
raw_name = input("Enter a soundcloud artist to analyze: ")
    ...
    for artist in artists_to_enqueue:
        try:
            print(
                "\t",
                "Enqueueing: %s (%s)"
                % (scac.getUsername(artist), scac.getUserid(artist)),
            )
            scac.addNode(artist)
